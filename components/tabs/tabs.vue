<template>
	<view class="tabs" :id="id" :style="{ background: bgColor }">
		<scroll-view scroll-x class="scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
			<view class="scroll-box" :class="{'tabs-scorll-flex': !isScroll}">
				<view class="tabs-item" :id="'tabs-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
				 :style="[tabItemStyle(index)]">
					{{ item[name] || item['name']}}
				</view>
				<view v-if="showBar" class="tabs-bar" :style="[tabBarStyle]"></view>
			</view>
		</scroll-view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async init() {
				// 获取tabs组件的尺寸信息
				let tabRect = await this.$getRect('#' + this.id);
				// tabs组件距离屏幕左边的宽度
			...
			getTabRect() {
				...
				for (let i = 0; i < this.list.length; i++) {
					// 只要size和rect两个参数
					query.select(`#tabs-item-${i}`).fields({
						size: true,
						rect: true
					});
				}
